...
describe('hackerNews.extractComments', function () {
  it('rejects null block', () => {
    const block = null
    const $ = {}

    try {
      const result = hackerNews.extractComments($, block)
  ...
  it('returns the correct comments', () => {
    ...
    try {
      const result = hackerNews.extractComments($, block)
      expect(result.comments).to.equal(expected)
    } catch (e) {
      expect(e).to.equal(undefined)
    }
  })

  it('returns nothing if a comments can´t be found', () => {
